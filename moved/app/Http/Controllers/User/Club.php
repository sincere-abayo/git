...
class Club extends Controller
{
    ...
    {
         $existInUser=User::where('user', $request->name)->first();
    if(!$existInUser)
    {
         return view('user.team.createTeam')->with('message','user '.$request->name.' Doesn`t exist in, invalid or wrong username  ');
    }
         $existInMember=member::where('member', $request->name)->first();
    if($existInMember)
    {
         return view('user.team.createTeam')->with('message','user '.$request->name.' Have joined club arleady, can not join two club');
    }
    $user = Auth::user();
    $userr=$user->user;
       $m=$request->name;

    ...
    <div class="container">
        ...
        <div class="card-container my-2">
            <div class="mainCard card1">
                <div class="title">
                    <h3 class="font-weight-bold text-3xl">{{$video->tittle}}</h3>
                </div>
                <div class="app-rej">
                    
                    <button class="btn btn-danger rejectBtn">reject</button>
                    <a href="{{route('admin.video.approve')}}?id={{$videoId}}" class="btn btn-primary">approve</a>
                        <span class="bg bg-success">{{$rejected}}</span>
                        <span class="bg bg-success">{{$message}}</span>
                                      </div>
                <div class="img">
                    <img src="{{ asset('video/'.$video->banner) }}" alt="">
                </div>  
                <div class="descp">
                    {{$video->description}}
                </div>
            ...
            <div class="mainCard card2">
                <div class="d-flex justify-content-between align-items-end ">
                    ...
                    <p class="text-primary font-semibold">{{$video->user}}</p>
                </div>
...
@endif
                <div class="views d-flex justify-content-between">
                    ...
                    <div class="view-icon font-weight-bold"><i class="lar la-eye"></i> {{$video->targeted_views}}</div>
                    </div>
                    <div class="reacted d-flex flex-column align-items-center">
                        <div class="font-semibold text-center">Reached Views</div>
                    <div class="view-icon font-weight-bold"><i class="lar la-eye"></i> {{$video->reached_views==NULL?0:$video->reached_views}}</div>
                ...
                <div class="views d-flex justify-content-between">
                 ...
                 <div class="paidMoney d-flex justify-between align-center my-2">
                    ...
                    <div class="text-blue-500 font-semibold">{{$video->targeted_duration}} days</div>
                </div>
                <div class="paidMoney d-flex justify-between align-center my-2">
                    <div class="font-semibold">Money Paid to vidoe</div>
                    <div class="text-blue-500 font-semibold">{{$video->total_paid}}$</div>
                </div>
                <div class="paidMoney d-flex justify-between align-center my-2">
                    <div class="font-semibold">Location</div>
                    <div class="text-blue-500 font-semibold">{{$video->location}}</div>
                </div>
...
<div class="reject bg-white rounded-xl shadow-lg py-2 px-4 " id="reject">
                ...
                <div class="text-center">
                    <form method="post" action="{{route('admin.video.reject')}}">
                        @csrf
